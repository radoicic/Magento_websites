<?php
	...
	class Downloadp extends \Magento\Framework\App\Action\Action
	{
		...
		public function execute() {
			...
			$spreadsheet->getActiveSheet()->getColumnDimension('C')->setWidth(90, 'px');
			$spreadsheet->getActiveSheet()->getColumnDimension('D')->setWidth(219, 'px');
			$spreadsheet->getActiveSheet()->getColumnDimension('E')->setWidth(59, 'px');
			$spreadsheet->getActiveSheet()->getColumnDimension('F')->setWidth(59, 'px');
			$spreadsheet->getActiveSheet()->getColumnDimension('G')->setWidth(59, 'px');
			$spreadsheet->getActiveSheet()->getColumnDimension('H')->setWidth(60, 'px');
			$spreadsheet->getActiveSheet()->getColumnDimension('I')->setWidth(60, 'px');
			$spreadsheet->getActiveSheet()->getColumnDimension('J')->setWidth(52, 'px');
			$spreadsheet->getActiveSheet()->getColumnDimension('K')->setWidth(5, 'px');
			...
			$spreadsheet->setActiveSheetIndex(0)
				...
				->setCellValue('E2', 'Costs and pricing Guide')
				->setCellValue('I2', 'SUPPLIED QTY')	
				->setCellValue('J2', 'QTY SOLD');	
			...
			$spreadsheet->getActiveSheet()->getStyle('B2:J3')->applyFromArray($styleHeader);
			...
			$spreadsheet->setActiveSheetIndex(0)->setCellValue('E3', 'Excl GST');
			$spreadsheet->setActiveSheetIndex(0)->setCellValue('F3', 'Incl GST');
			$spreadsheet->setActiveSheetIndex(0)->setCellValue('G3', 'RRP');
...
/***********/
				...
				$spreadsheet->setActiveSheetIndex(0)
					...
					->setCellValue('E'.$s, $this->priceHelper->currency($product->getPriceInfo()->getPrice('final_price')->getAmount()->getBaseAmount(), true, false))
					->setCellValue('F'.$s, $this->priceHelper->currency($this->taxHelper->getTaxPrice($product, $product->getPriceInfo()->getPrice('final_price')->getAmount()->getBaseAmount(), true), true, false))
					->setCellValue('G'.$s, $this->priceHelper->currency($this->taxHelper->getTaxPrice($product, $product->getFinalPrice(), true), true, false))
//					->setCellValue('G'.$s, $product->getPriceInfo()->getPrice('regular_price')->getAmount())
					->setCellValue('I'.$s, $product->getQty());				
...
*/
				$spreadsheet->getActiveSheet()->getStyle('B'.$s.':J'.$s)->applyFromArray($styleCell);
				 //break;
				 $s++;
			}
...
/***********/
			...
			header('Content-Disposition: attachment;filename="Consignment Stock With Prices -'.gmdate('d M Y H:i') .'.xls"');
