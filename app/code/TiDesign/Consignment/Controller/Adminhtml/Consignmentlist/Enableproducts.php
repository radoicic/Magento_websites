 ...
 * Class MassDelete
 */
class Enableproducts extends \Magento\Backend\App\Action
{
    /**
     * @var Filter
     */
    ...
    public function execute()
    {
		...
		if (!empty($postData)) {
			...
			$dlist = explode(',', $model->getTdDisabled());
			
			
			foreach($postData['enable_id'] as $i => $val){
				unset($dlist[array_search($val, $dlist)]);
			}
			$data['td_disabled'] = implode(',' , array_filter($dlist));
			
			$model->setData($data);
			try {
				$this->consignmentlistRepository->save($model);
				$this->messageManager->addSuccessMessage(__('Products enabled'));
