...
class StockEmailProcessor
{
                ...
                try {
                    ...
                    if ($product) {
                        ...
                        if (in_array($product->getTypeId(), ['bundle', 'configurable', 'grouped'])) {
                            ...
                            $qty = 0;
                            $parentStock = $this->getStockStatus($product->getSku(), $stockId);
                            $childStock = false;
                            ...
                            $status = $this->getStockStatus($product->getSku(), $stockId);
                            $qty = $salableQty->execute($product->getSku(), $stockId);
            ...
            if ($this->isConfigurableAndHasChild($product->getTypeId(), $productId)) {
                ...
                foreach ($this->productRelation[$productId] as $childProductId) {
                    ...
                    if ($this->isReadyToMail($childIsInStock, $qtyToSend, $childQty)) {
                        ...
                        $this->stockResource->updateStock($stockId, ['product_sku' => $productSku, 'status' => 0]);
                        break;
                    }
                }
            } else {
                ...
                if ($productSku !== $stockData['product_sku']) {
                    $this->stockResource->updateStock($stockId, ['product_sku' => $productSku]);
                }
            }
     ...
     * @param int $websiteId
     * @param array|null $dataStock
     */
    protected function listEmails($products, $email, $customerNames, $emailSend, $websiteId, $dataStock = null)
    {
        try {
            $productIds = [];
            $status = 0;
            ...
            foreach ($products as $productData) {
                if (isset($productData['is_salable']) &&
                    ...
                    $stockId = (int)$productData['stockId'];
                    if (isset($dataStock[$stockId]['status'])) {
                        $status = (int)$dataStock[$stockId]['status'];
                    }
                    if (!$status) {
    ...
    private function createSkeletonStockAlert($stockModel)
    {
        return [
            ...
            'product_sku' => $stockModel['product_sku'],
            'product_id' => $stockModel['product_id'],
